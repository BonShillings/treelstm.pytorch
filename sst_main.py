...
def main():
    ...
    sst_vocab_file = os.path.join(args.data, 'sst.vocab')
    if not os.path.isfile(sst_vocab_file):
        token_files_sents = [os.path.join(split, 'sents.toks') for split in [train_dir, dev_dir, test_dir]]
        sst_vocab_file = os.path.join(args.data, 'sst.vocab')
        utils.build_vocab(token_files_sents, sst_vocab_file)

    # get vocab object from vocab file previously written
    vocab = Vocab(filename=sst_vocab_file,
                  data=[Constants.PAD_WORD, Constants.UNK_WORD,
                        Constants.BOS_WORD, Constants.EOS_WORD])
    logger.debug('==> SST vocabulary size : %d ' % vocab.size())
